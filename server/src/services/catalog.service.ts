...
export async function getEmptyCatalogEntries(userId: string) {
  const result = await sql<{
    ...
    WHERE cc.user_id = ${userId}
      ...
      -- A part with raw_purchases pointing at it is "in use" even with zero
      -- inspection lines yet (cards still pending allocation in the lot).
      AND NOT EXISTS (
        SELECT 1 FROM raw_purchases rp
        WHERE rp.catalog_id = cc.id
