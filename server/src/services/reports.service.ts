  ...
  if (groupBy === 'platform') {
    rows = (await db
      ...
      .select([
        ...
        sql<number>`SUM(CASE WHEN s.platform = 'card_show' THEN 0 ELSE s.platform_fees + s.shipping_cost END)::int`.as('total_fees'),
...
export async function getCardShowBreakdown(userId: string, showId: string) {
  ...
  const result = await db
    ...
    .select([
      ...
      sql<number>`COALESCE(SUM(CASE WHEN s.platform = 'card_show' THEN 0 ELSE s.platform_fees + s.shipping_cost END) FILTER (WHERE ${slabCheck}), 0)::int`.as('slab_fees'),
      ...
      sql<number>`COALESCE(SUM(CASE WHEN s.platform = 'card_show' THEN 0 ELSE s.platform_fees + s.shipping_cost END) FILTER (WHERE NOT ${slabCheck}), 0)::int`.as('raw_fees'),
