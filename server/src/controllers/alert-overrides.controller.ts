...
export async function getStaleEbay(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const data = await svc.getStaleEbayListingsFull(req.dataUserId, days);
    res.json({ data });
  } catch (err) { next(err); }
}
...
export async function getStaleCardShow(req: Request, res: Response, next: NextFunction) {
  try {
    const days = z.coerce.number().int().min(1).default(DAYS_DEFAULT).parse(req.query.days);
    const data = await svc.getStaleCardShowFull(req.dataUserId, days);
    res.json({ data });
  } catch (err) { next(err); }
}
...
export async function muteAlert(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    await svc.muteAlert(req.dataUserId, entity_type, entity_id);
    res.json({ ok: true });
  } catch (err) { next(err); }
}
...
export async function ignoreAlert(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    await svc.ignoreAlert(req.dataUserId, entity_type, entity_id);
    res.json({ ok: true });
  } catch (err) { next(err); }
}
...
export async function resetAlert(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    await svc.resetAlert(req.dataUserId, entity_type, entity_id);
    res.json({ ok: true });
  } catch (err) { next(err); }
}
