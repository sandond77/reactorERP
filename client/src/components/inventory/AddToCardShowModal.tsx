...
interface SelectedCard extends SlabOption {
...
export function AddToCardShowModal({ onSuccess }: { onSuccess: () => void }) {
  ...
  function toggleRow(row: SlabOption) {
    setSelected((prev) => {
            ...
            {selectedList.map((card) => (
              <div key={card.id} className="grid grid-cols-[1fr_8rem_10rem_2.5rem] gap-x-4 px-4 py-3.5 border-b border-zinc-800/60 last:border-0 items-center">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-zinc-500 mt-0.5">{card.grade_label} · {card.company} · #{card.cert_number}</p>
                </div>
                ...
                <button
                  onClick={() => setSelected((prev) => { const next = new Map(prev); next.delete(card.id); return next; })}
    ...
    <div className="flex flex-col gap-4">
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search by card name or cert…"
          ...
          {isLoading ? (
            ...
            return (
              <div
                ...
                className={cn(
                  'grid grid-cols-[2.5rem_1fr_10rem_6rem] gap-x-4 px-4 py-3.5 border-b border-zinc-800/60 last:border-0 items-center cursor-pointer transition-colors',
                  isChecked ? 'bg-indigo-600/10' : 'hover:bg-zinc-800/40'
                )}
              >
                <div className={cn(
