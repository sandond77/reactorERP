...
- **Run `npm run lint` before committing** — from `client/`. Zero errors AND zero warnings expected. If your changes add new lint hits, fix them in the same commit (don't push and defer). Use existing helpers like `apiErrorMessage` in `client/src/lib/api.ts` instead of `(err as any)?.response?.data?.error`; if you find yourself reaching for `as any` in an axios error handler, adopt the helper instead. For genuinely unavoidable cases (zod-resolver ↔ react-hook-form type gap, sync-with-external-state effect resets, mount-only listeners with function deps), use a targeted `eslint-disable-next-line <rule>` with a comment explaining why — never a file-wide disable.
